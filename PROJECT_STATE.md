...
- Xcode is installed at `/Applications/Xcode.app`; using `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer`, `xctrace` became available and captured `benchmarks/baseline/2026-08-27-metal-system-trace.trace`. The target exited normally, trace duration was 2.108253 s, and the template was `Metal System Trace`. GPU counters and shader timeline were disabled, so this is an execution trace artifact, not yet a kernel-counter analysis.
- Extended trace captured at `benchmarks/baseline/2026-08-27-metal-extended.trace` with `Metal Application` and a 10 s limit. The target exited normally, trace duration was 3.969234 s, and the artifact is 77 MB. Instruments still reports `Counter Set: (null)` and `Shader Timeline: Disabled`; adding the instrument did not enable GPU counters on this machine/configuration.
...
- Existing Metal trace analysis: `xctrace export --toc` succeeded for `benchmarks/baseline/2026-08-27-metal-system-trace.trace`. It confirms `llama-cli` exited 0 on the M3, trace duration 2.108253 s, Metal System Trace template, and the exact model/CLI arguments. The trace records `Counter Set: (null)` and `Shader Timeline: Disabled`, so it provides lifecycle/execution evidence only, not GPU counters, shader timings, or a kernel speedup.
